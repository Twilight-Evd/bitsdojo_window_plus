...
class BitsdojoWindowImpl : BitsdojoWindowGtk {
   public:
    ...
    void setCustomFrame(gboolean value) override {
        if (false == value) {
            return;
        }
    ...
    void setAlwaysOnTop(gboolean value) override {
        gtk_window_set_keep_above(this->handle, value);
    }

    void setWindowTitleBarButtonVisibility(int button, gboolean visible) override {
        // DesktopWindowButton: 0=close, 1=minimize, 2=zoom
        if (button == 0) {
            gtk_window_set_deletable(this->handle, visible);
        } else if (button == 1) {
            // Note: GTK doesn't have a direct "hide minimize" that works easily with all WMs,
            // but we can try setting hints.
        }
        
        // If using HeaderBar, we can control layout
        GtkWidget* titlebar = gtk_window_get_titlebar(this->handle);
        if (titlebar && GTK_IS_HEADER_BAR(titlebar)) {
            // Rebuilding decoration-layout would be complex here without storing state.
            // For now, we mainly rely on bitsdojo custom buttons in Flutter.
        }
    }

    void setBackgroundEffect(int effect) override {
        auto window = GTK_WIDGET(this->handle);
        auto screen = gtk_window_get_screen(this->handle);
        
        if (effect == 1 || effect == 2) { // Transparent or Acrylic
            gtk_widget_set_app_paintable(window, TRUE);
            
            auto *provider = gtk_css_provider_new();
            // Apply transparency to window and any container that might be opaque
            gtk_css_provider_load_from_data(provider, 
                "window, grid, box, stack, eventbox { background: none; background-color: rgba(0,0,0,0); }", 
                -1, NULL);
            ...
            if (this->event_box) {
                gtk_widget_set_app_paintable(this->event_box, TRUE);
                gtk_style_context_add_provider(gtk_widget_get_style_context(this->event_box), 
                                             GTK_STYLE_PROVIDER(provider), 
                                             GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
            }
            ...
            if (this->event_box) {
                gtk_widget_set_app_paintable(this->event_box, FALSE);
            }
        }
