...
void BitsdojoWindowPlugin::RegisterWithRegistrar(
  ...
  if (window) {
    ...
    // Get pending name/arguments from MultiWindowManager
    std::string name = MultiWindowManager::GetInstance().GetPendingName();
    if (!name.empty()) {
      args[flutter::EncodableValue("name")] = flutter::EncodableValue(name);
    }
    std::string arguments =
        MultiWindowManager::GetInstance().GetPendingArguments();
    if (!arguments.empty()) {
